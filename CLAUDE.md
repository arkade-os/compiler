...
<conventions>
  ...
  <patterns>
    <do>
      ...
      - The ABI is one `functions[]` of spend groups: each entry has an optional `arkade` covenant object and a `leaves[]` array of L1 tapleaves. Assert covenant ASM against `arkade.asm`, tapleaf ASM against `leaves[].asm`. There is no `serverVariant`/`witnessSchema` shape anymore.
      - Strip or ignore `updatedAt` when comparing expected vs actual JSON in tests.
      - Keep placeholder format `<name>` in emitted ASM. Co-signer placeholders are `<SERVER_KEY>` (arkd operator) and `<EMULATOR_KEY:funcName>` (emulator key tweaked by `funcName`'s covenant hash).
      - Cooperative signing is automatic: a covenant `function` with no matching `tapscript` gets a synthesized default collaborative leaf `checkMultisig([server, tweak(emulator, fn)], [serverSig, emulatorSig], 2)`. Only write an explicit `function <name>(...) tapscript { ... }` when you need extra signers, a hashlock, or a custom timelock.
      - Unilateral exit is explicit: add a standalone CSV `tapscript` (`require(older(exit)); require(checkSig(ownerSig, owner));`) when the contract needs an exit path. Keep `int exit` (and `int renew`) as constructor parameters so the playground can set them; reference them via `older(...)` / `tx.time >= ...`. A contract with no exit leaf is valid (it simply has no unilateral exit).
    ...
    <dont>
      ...
      - Do not edit generated playground artifacts manually (`playground/contracts.js`, `playground/pkg/*`); they are gitignored and regenerated by the deploy workflows.
...
<memory>
  <project_decisions>
    - [2026-06-30] ABI is one `functions[]` of spend groups: `{ name, arkade?: { inputs, asm }, leaves: [{ name, witness, asm }] }`. Replaces the old `serverVariant=true/false` two-variant shape (removed). Closure classification stays internal — no `kind`/`closure` field in the ABI.
    - [2026-06-30] Custom L1 tapscripts: `function <name>(<inputs>) tapscript { require(...)* }` defines a taproot leaf. It must assemble to one of arkd's 5 closures (Multisig / CSVMultisig / CLTVMultisig / ConditionMultisig / ConditionCSVMultisig). Source order is `condition? · timelock? · multisig`. Multisig is always N-of-N (threshold == key count). Forfeit closures must include `server`. See `src/compiler/tapscript.rs` and `docs/tapscript-leaves-spec.md`.
    - [2026-06-30] `options { ... }` REMOVED from the language. Cooperative signing = synthesized default leaf `[server, tweak(emulator, fn)]` for any covenant function with no matching tapscript. Exit = explicit standalone CSV tapscript. `exit`/`renew` are ordinary `int` constructor params referenced by `older(...)`/`tx.time >=`.
