...
The failure mode that follows from this shape: a compromised oracle can trigger
or withhold the one agreed settlement, and nothing else.

---

## The exit

An Arkade VTXO needs a way out when the operator stops co-signing. `unilateral`
is that way out: after `exit`, party A and party B spend together. Neither
party can exit alone. The oracle key is not in the leaf.
